    ...
    for (m in seq_along(gps)){
        ...
        for (cn in seq_along(contexts)){
            ...
            if (file.exists(fn1)) {
                StateCall <- fread(fn1)
                df1 <- subset(
                    StateCall,, which(colnames(StateCall) %in% c(
                        'seqnames','start','end', gp1, gp2)))
                fwrite(x=df1, file=paste0(
                    out.dir, "/", contexts[cn], "_", 
                    out.name, "_StateCalls.txt"), quote=FALSE, row.names=FALSE, 
                    col.names=TRUE, sep="\t")
            }
            ...
            if (file.exists(fn2)) {
                rcMethlvl <- fread(fn2)
                df2 <- subset(
                    rcMethlvl,, which(colnames(rcMethlvl) %in% c(
                        'seqnames', 'start', 'end', gp1, gp2)))
                fwrite(x=df2, file=paste0(
                    out.dir, "/", contexts[cn], "_", out.name, 
                    "_rcMethlvl.txt"), quote=FALSE, row.names=FALSE, 
                    col.names=TRUE, sep="\t")
            }
            if (postMax.out==TRUE){
                ...
                if (file.exists(fn3)) {
                    postMax <- fread(fn3)
                    df3 <- subset(
                        postMax,, which(colnames(postMax) %in% c(
                            'seqnames', 'start', 'end', gp1, gp2)))
                    fwrite(
                        x=df3,file=paste0(
                            out.dir, "/", contexts[cn], "_", 
                            out.name, "_postMax.txt"), quote=FALSE, 
